...
// plug this site in as a provider. Auth is via the per-user API key.
const express = require('express');
const pool = require('../db/pool');
const pricing = require('../services/pricing');
const orderService = require('../services/orders');
const { apiLimiter } = require('../middleware/rateLimit');
const { isValidHttpUrl, statusLabel } = require('../utils/helpers');
...
router.post('/api/v2', apiLimiter, express.urlencoded({ extended: false }), async (req, res) => {
  try {
    ...
    const [[user]] = await pool.query('SELECT * FROM users WHERE api_key = ?', [key]);
    if (!user || user.banned_at) return res.json({ error: 'Invalid API key' });
    ...
    if (action === 'services') {
      const [services] = await pool.query(
        'SELECT * FROM services WHERE enabled = 1 AND deleted = 0 ORDER BY platform, category');
    ...
    if (action === 'add') {
      ...
      if (quantity < service.min_qty || quantity > service.max_qty) {
        return res.json({ error: `Quantity must be between ${service.min_qty} and ${service.max_qty}` });
      }
      try {
        const { orderId } = await orderService.placeOrder(user.id, service, link, quantity);
    ...
    if (action === 'status') {
      ...
      for (const id of ids) {
        ...
        results[id] = order
          ? {
              charge: Number(order.charge_php).toFixed(2),
